 ...
 * @dev Supports both Mantle (native) and MNT (ERC20) payments
 */
contract NFTMarketplace is ReentrancyGuard {
    ...
    event ItemSoldWithMNT(uint256 indexed tokenId, address indexed buyer, address indexed seller, uint256 price);
    event ItemCanceled(uint256 indexed tokenId, address indexed seller);

    /**
     * @notice Constructor sets the NFT contract address and MNT token address
     * @param _nftContract Address of the RunnerBadge contract
     * @param _MNTToken Address of the MNT token contract (0xdE9e4C3ce781b4bA68120d6261cbad65ce0aB00b for Mantle Sepolia)
     */
    constructor(address _nftContract, address _MNTToken) {
    ...
    /**
     * @notice Buy a listed NFT with MNT
     * @param tokenId The ID of the NFT to buy
     * @param MNTAmount The amount of MNT to pay (must match listing price)
     */
    function buyItemWithMNT(uint256 tokenId, uint256 MNTAmount) external nonReentrant {
        Listing memory listing = listings[tokenId];

        require(listing.isActive, "Item not listed");
        require(MNTAmount >= listing.price, "Insufficient payment");
        require(nftContract.ownerOf(tokenId) == listing.seller, "Seller no longer owns NFT");
        ...
        // Transfer MNT from buyer to seller (exact amount)
        MNTToken.safeTransferFrom(msg.sender, listing.seller, listing.price);

        // Refund excess payment if any (transfer back to buyer)
        if (MNTAmount > listing.price) {
            MNTToken.safeTransfer(msg.sender, MNTAmount - listing.price);
        }

        emit ItemSoldWithMNT(tokenId, msg.sender, listing.seller, listing.price);
    }
