...
export function GameOverModal() {
  ...
  return (
    <div 
      className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 pointer-events-auto p-2 sm:p-4"
      onClick={() => {
        // Only allow closing if not saving - prevent accidental closure during transaction
        if (!isSavingSession && !saveSuccess) {
          playSound('button');
          restartGame();
        }
      }}
    >
      <div 
        className="nes-container with-title is-centered pixel-art max-w-sm sm:max-w-md lg:max-w-lg w-full" 
        style={{ backgroundColor: 'white', border: '3px solid #000' }}
